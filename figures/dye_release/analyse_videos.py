...
def compute_dye_release_info(videofilename):
    ...
    while ret:
        ...
        ## mask in arena and not droplet
        mask = cv2.bitwise_and(droplet_mask, arena_mask)

        ## get the hue pixel value that are in arena and not a droplet
        ...
        hist_hue.append(hist_data.tolist())
        hist_position.append(positions.tolist())

        ratio_above_threshold.append(np.sum(to_process_frame > HUE_THRESHOLD) / np.float(np.size(to_process_frame)))
    ...
    results['hist_hue'] = hist_hue
    results['hist_position'] = hist_position
    results['ratio_above_threshold'] = ratio_above_threshold
    return results
...
if __name__ == '__main__':
    ...
    for video_filename in video_files:
        ...
        save_filename = os.path.join(save_folder, 'dye_release_info.json')

        results = compute_dye_release_info(video_filename)
